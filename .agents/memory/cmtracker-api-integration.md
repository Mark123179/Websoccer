...
- A **live** key only works from **registered IP addresses**. Production now runs on the Hetzner server with the FIXED IP `49.13.5.151`, so the live import runs DIRECTLY on the server (in the `web` container) — **no proxy needed**; that server IP is the one to register at cmtracker. From any unregistered IP (incl. the Replit dev env) the API returns **401 by design** — a 401/403 in dev is expected, not a key bug, and live cannot be tested from Replit.
...
- Full technical doc: `docs/cmtracker_integration.md` (mapping, dry-run result, sandbox limit, live workflow on the server §6).
