...
2. **Kein Minus.** Der Kontostand kann nie negativ werden. Buchungen, die ins Minus führen würden, schlagen fehl bzw. werden durch Cashflow-Design vermieden (siehe Kap. 12).
...
| KADER_MAX_BASIS | 60 (Umfeld-erweiterbar) | 9 |
...
Faktoren: Beliebtheit (aus Fanbeliebtheits-System) 0,7–1,2 · Gegner-Attraktivität (Kader-MW-Verhältnis + Tabellennähe + Derby-Flag) 0,85–1,3 · Preisfaktor je Kategorie `(Referenzpreis / Preis)^PREIS_ELASTIZITAET`, geklemmt auf 0,5–1,3.
...
**Kaderplatz-Voraussetzung:** Käufe (KI wie Manager) nur bei freiem Kaderplatz. Standard-Kaderlimit 60 Spieler, erweiterbar über Stadionumfeld-Ausbauten (`KADER_MAX_BASIS = 60` in EconomyParameter; Erweiterungslogik im Umfeld-Modul).
...
**Auktionen** (bestehendes System) sind die einzige *dosierbare* Senke: Zuschlagserlöse werden vollständig vernichtet (Buchungstyp AUKTION). Volumen und Qualität der Auktionsspieler pro Saison steuert der Admin — das gezielte Ventil gegen Geldüberhänge, das dort absaugt, wo Geld liegt, freiwillig und mit Spielspaß.
...
### 12.3 Monitoring (Datencenter / Admin)
...
Begründung: Ein absolut gleiches Budget wäre real extrem ungleich (40 % Kaderwert für Elversberg, Rundungsfehler für City). Die Umsatzquote gibt jedem Verein dieselbe Handlungsfähigkeit in seiner Preisklasse. Bewusst **nicht** Kader-MW-basiert — sonst bekämen genau die strukturellen Defizitvereine (PSG, City) die dicksten Polster. Randfälle: Neue Ligen erhalten Startbudgets nach derselben Formel zum Launch-Zeitpunkt (kein Sonderfall). Vereinsübernahmen im laufenden Betrieb erhalten **kein** Startbudget — der neue Manager erbt den Ledger-Kontostand des Vereins.
...
**Phase 3 — Stadionökonomie:** Kategorien & Preise (Manager-UI), volle Nachfrageformel, Ausbau-Bänder, Unterhalt, Umfeld-Anbindung.
